...
def agent_router(state: Dict[str, Any]) -> str:
    # 假设 state 中有一个标记，标记是否需要检索
    if state.get("force_web_search"):
        return "web_search"
    # 如果不需要检索，直接跳到生成节点
    if state.get("skip_retrieval"):
        return "generate"
    return "retrieve"  # 如果需要检索，则跳到 retrieve 节点
...
workflow.add_conditional_edges(
    "agent",
    tools_condition,          # ← 用内置的 tools_condition
    {
        "tools": "retrieve",  # LLM 调用了工具 → 检索
        END: "generate",      # LLM 没调工具 → 直接生成
    },
)
